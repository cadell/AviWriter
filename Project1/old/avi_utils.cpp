...
HRESULT AddAviFrame(HAVI avi, HBITMAP hbm, unsigned char* data)
{
	...
	bitmapInfoHeader.bmiHeader.biSizeImage = 0;// 4690000;//0;
	...
	HDC hdc = GetDC(NULL);
	HBITMAP holdBitmap = CreateDIBSection(hdc, &bitmapInfoHeader, DIB_RGB_COLORS, (void**)data, NULL, 0);
	if (avi == NULL) return AVIERR_BADHANDLE;
	if (holdBitmap == NULL) return AVIERR_BADPARAM;
	DIBSECTION dibs; int sbm = GetObject(holdBitmap, sizeof(dibs), &dibs);
	...
	HRESULT hr = AVIStreamWrite(au->psCompressed, au->nframe, 1, data, dibs.dsBmih.biSizeImage, AVIIF_KEYFRAME, NULL, NULL);
	if (hr != AVIERR_OK) { au->iserr = true; return hr; }
